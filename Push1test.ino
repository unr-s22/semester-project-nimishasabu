...
unsigned char *port_L = (unsigned char *) 0x10B; // for fan stepButton rLED and gLED
...
int stepButtonState = 0;        // Stepper motor button High or low
...
unsigned char respin = A5;                // analog water sensor - pin used
...
void setup()
{
  ...
  *DDR_L |= 0b01000000;                           // stepMotor - input

  // 
  *EICRA_1 |= 0b00000010;                         // set interrupt trigger to CHANGE
  ...
  *port_L &= (0 << PINL4) & (0 << PINL0) & (0 << PINL2); // set fan gled rled to low
  ...
  if (disabledState==1){
    ...
    *port_L |= (1 << PINL0); // turn the green led on
    ...
    stepButtonState = digitalRead(43);
